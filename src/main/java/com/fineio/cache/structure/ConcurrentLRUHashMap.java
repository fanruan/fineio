...
public class ConcurrentLRUHashMap<K, V> extends AbstractMap<K, V> implements
     ...
     * Segement默认最大数
     */
    static final int DEFAULT_SEGMENT_MAX_CAPACITY = 100;
    /**
    ...
    transient List<AutoRemoveCallBack<K, V>> callBacks;
    ...
    public ConcurrentLRUHashMap(int segmentCapacity, float loadFactor,
        ...
        this.callBacks = new ArrayList<AutoRemoveCallBack<K, V>>();
        if (null != callBack) {
            this.callBacks.add(callBack);
        }

        for (int i = 0; i < this.segments.length; ++i) {
            this.segments[i] = new Segment<K, V>(segmentCapacity, loadFactor, this, this.callBacks);
        }
    }
     ...
     * 使用指定参数，创建一个ConcurrentLRUHashMap
     *
     * @param segmentCapacity Segment最大容量
     * @param loadFactor      加载因子
     */
    public ConcurrentLRUHashMap(int segmentCapacity, float loadFactor, AutoRemoveCallBack<K, V> callBack) {
        this(segmentCapacity, loadFactor, DEFAULT_CONCURRENCY_LEVEL, callBack);
    }
     ...
     * @param segmentCapacity Segment最大容量
     */
    public ConcurrentLRUHashMap(int segmentCapacity, AutoRemoveCallBack<K, V> callBack) {
        this(segmentCapacity, DEFAULT_LOAD_FACTOR, DEFAULT_CONCURRENCY_LEVEL, callBack);
    }
    ...
    public ConcurrentLRUHashMap() {
        this(DEFAULT_SEGMENT_MAX_CAPACITY, DEFAULT_LOAD_FACTOR,
                DEFAULT_CONCURRENCY_LEVEL, null);
    }
    ...
    public V poll() {
        for (Segment<K, V> segment : segments) {
            HashEntry<K, V> entry = segment.header.linkNext;
            if (null != entry) {
                return entry.value;
            }
        }
    ...
    static final class Segment<K, V> extends ReentrantLock implements Serializable {
        ...
        Segment(int maxCapacity, float lf, ConcurrentLRUHashMap<K, V> lruMap, List<AutoRemoveCallBack<K, V>> callBacks) {
        ...
        void removeEldestEntry() {
            if (count > this.maxCapacity) {
                ...
                V value = remove(eldest.key, eldest.hash, null);
                for (AutoRemoveCallBack autoRemoveCallBack : callBacks) {
                    autoRemoveCallBack.callback(eldest.key, value);
                }
            }
        }
