...
import com.fineio.v2.io.file.EditIOFile;
import com.fineio.v2.io.file.IOFile;
import com.fineio.v2.io.file.ReadIOFile;
import com.fineio.v2.io.file.WriteIOFile;
...
public class FineIOTest extends TestCase {
    ...
    public void testCreateEditIOFilePlus() throws Exception {
        byte len = (byte) (Math.random() * 100d);
        final byte[] res = new byte[16];
        Bits.putInt(res, 0, len * 2);
        res[8] = len;
    ...
    public void testEdit() throws Exception {
        ...
        System.out.println(ifile.fileSize());
        System.out.println(dfile.fileSize());
        file.closeAndClear();
        ifile.closeAndClear();
        dfile.closeAndClear();
        MemoryLeakTest.assertSizeMemory(0);
...
//        }
        ...
        long size = dfile.fileSize();
        dfile.close();
//        dfile2.close();
//        dfile3.close();
        MemoryLeakTest.assertSizeMemory(size);
        CacheManager.clear();
    }
    ...
    public void testDirectAccess() {
        ...
        FineIO.put(ie, len / 2, 100);
        assertEquals(100, FineIO.getInt(ie, len / 2));
        ie.close();
        ir = FineIO.createIOFile(connector, uri, FineIO.MODEL.READ_INT_DIRECT);
        assertEquals(100, FineIO.getInt(ir, len / 2));
        ir.close();
        MemoryLeakTest.assertSizeMemory(0);
