...
import com.fineio.v3.cache.core.testing.CacheSpec.Stats;
import com.fineio.v3.cache.core.testing.CacheSpec.Writer;
import com.fineio.v3.cache.core.testing.CacheValidationListener;
import com.fineio.v3.cache.testing.Threads;
...
public final class MultiThreadedTest {
    ...
    List<BiConsumer<LoadingCache<Integer, Integer>, Integer>> operations = ImmutableList.of(
            ...
            (cache, key) -> {
                cache.getAll(ImmutableList.of(key));
            },
            (cache, key) -> {
                cache.refresh(key);
            },
            ...
            (cache, key) -> {
                cache.getAllPresent(ImmutableList.of(key));
            },
            (cache, key) -> {
                cache.put(key, key);
            },
            (cache, key) -> {
                cache.putAll(ImmutableMap.of(key, key));
            },
            (cache, key) -> {
            ...
            (cache, key) -> {
                Preconditions.checkState(cache.estimatedSize() >= 0);
            },
            (cache, key) -> {
                cache.stats();
            },
            ...
            (cache, key) -> {
                Preconditions.checkState(cache.asMap().size() >= 0);
            },
            (cache, key) -> {
                cache.asMap().get(key);
            },
            ...
            (cache, key) -> {
                cache.asMap().putAll(ImmutableMap.of(key, key));
            },
            (cache, key) -> {
            ...
            (cache, key) -> {
                cache.asMap().computeIfAbsent(key, k -> k);
            },
            (cache, key) -> {
                cache.asMap().computeIfPresent(key, (k, v) -> v);
            },
            (cache, key) -> {
                cache.asMap().compute(key, (k, v) -> v);
            },
            (cache, key) -> {
                cache.asMap().merge(key, key, (k, v) -> v);
            },
            (cache, key) -> { // expensive so do it less frequently
                int random = ThreadLocalRandom.current().nextInt();
                if ((random & 255) == 0) {
            ...
            (cache, key) -> {
                cache.asMap().keySet().toArray(new Object[cache.asMap().size()]);
            },
            (cache, key) -> {
                cache.asMap().values().toArray(new Object[cache.asMap().size()]);
            },
            (cache, key) -> {
    ...
    List<BiConsumer<AsyncLoadingCache<Integer, Integer>, Integer>> asyncOperations = ImmutableList.of(
            ...
            (cache, key) -> {
                cache.get(key, (k, e) -> CompletableFuture.completedFuture(key));
            },
            (cache, key) -> {
                cache.get(key);
            },
